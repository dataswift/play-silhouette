...
object CookieSecret {
  ...
  def unserialize(
      str: String,
      signer: Signer,
      crypter: Crypter): Try[CookieSecret] =
  ...
  private def buildSecret(str: String): Try[CookieSecret] =
    Try(Json.parse(str)) match {
      case Success(json) =>
        json.validate[CookieSecret].asEither match {
...
/**
  ...
  */
case class CookieSecret(
    value: String,
    expirationDate: DateTime)
    extends OAuth1TokenSecret {
...
class CookieSecretProvider @Inject() (
  ...
  override def build[B](
      ...
      ec: ExecutionContext): Future[CookieSecret] =
  ...
  override def retrieve[B](
    ...
    request.cookies.get(settings.cookieName) match {
      case Some(cookie) =>
        CookieSecret.unserialize(cookie.value, signer, crypter) match {
...
object CookieSecretProvider {
  ...
  val InvalidSecretFormat =
    "[Silhouette][CookieSecretProvider] Cannot build token secret because of invalid Json format: %s"
