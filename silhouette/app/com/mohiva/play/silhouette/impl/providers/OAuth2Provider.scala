...
trait OAuth2Provider extends SocialStateProvider with OAuth2Constants with Logger {
  ...
  /**
    ...
    */
  def authenticate[S <: SocialStateItem, B](
      userState: S
    )(implicit
      format: Format[S],
  ...
  /**
    ...
    */
  def handleFlow[L, R, B](
      left: => Future[L]
    )(right: String => Future[R]
    )(implicit
  ...
  /**
    ...
    */
  protected def handleAuthorizationFlow[B](
    ...
    stateHandler.state.map { state =>
      ...
      val params = settings.scope.foldLeft(
        List((ClientID, settings.clientID),
             (ResponseType, Code)
        ) ++ stateParam ++ settings.authorizationParams.toList ++ redirectParam
      ) {
        case (p, s) => (Scope, s) :: p
      }
  ...
  protected def getAccessToken(code: String)(implicit request: RequestHeader): Future[OAuth2Info] = {
    ...
    val params = Map(ClientID -> Seq(settings.clientID),
                     ClientSecret -> Seq(settings.clientSecret),
                     GrantType -> Seq(AuthorizationCode),
                     Code -> Seq(code)
      ) ++ settings.accessTokenParams.mapValues(Seq(_)) ++ redirectParam.toMap.mapValues(Seq(_))
