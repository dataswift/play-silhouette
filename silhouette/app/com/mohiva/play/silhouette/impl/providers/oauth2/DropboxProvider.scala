...
trait BaseDropboxProvider extends OAuth2Provider {
  ...
  override protected def buildProfile(authInfo: OAuth2Info): Future[Profile] =
    httpLayer.url(urls("api")).withHttpHeaders(AUTHORIZATION -> s"Bearer ${authInfo.accessToken}").get().flatMap {
      response =>
...
class DropboxProfileParser extends SocialProfileParser[JsValue, CommonSocialProfile, OAuth2Info] {
  ...
  override def parse(
    ...
    Future.successful {
      ...
      CommonSocialProfile(loginInfo = LoginInfo(ID, userID.toString),
                          firstName = firstName,
                          lastName = lastName,
                          fullName = fullName
      )
...
object DropboxProvider {
  ...
  val SpecifiedProfileError =
    "[Silhouette][%s] Error retrieving profile information. Error message: %s, status code: %s"
