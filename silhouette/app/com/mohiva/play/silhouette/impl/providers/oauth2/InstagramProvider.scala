...
class InstagramProfileParser extends SocialProfileParser[JsValue, CommonSocialProfile, OAuth2Info] {
  ...
  override def parse(
    ...
    Future.successful {
      ...
      CommonSocialProfile(loginInfo = LoginInfo(ID, userID), fullName = fullName, avatarURL = avatarURL)
    }
}
...
object InstagramProvider {
  ...
  val SpecifiedProfileError =
    "[Silhouette][%s] Error retrieving profile information. Error code: %s, type: %s, message: %s"
