...
trait OAuth1Provider extends SocialProvider with OAuth1Constants with Logger {
  ...
  if (!service.use10a)
    throw new RuntimeException(
      "You must use the the 1.0a specification to address the session fixation " +
  ...
  def authenticate[B]()(implicit request: ExtractableRequest[B]): Future[Either[Result, OAuth1Info]] =
    request.extractString(Denied) match {
      ...
      case None =>
        request.extractString(OAuthVerifier) -> request.extractString(OAuthToken) match {
          ...
          case (Some(verifier), Some(token)) =>
            tokenSecretProvider.retrieve.flatMap { tokenSecret =>
              service
                .retrieveAccessToken(OAuth1Info(token, tokenSecret.value), verifier)
                .map { info =>
                  Right(info)
                }
                .recover {
          ...
          case _ =>
            service
              .retrieveRequestToken(resolveCallbackURL(settings.callbackURL))
              .flatMap { info =>
...
trait OAuth1Service {
  ...
  def retrieveAccessToken(
      ...
      verifier: String
    )(implicit ec: ExecutionContext): Future[OAuth1Info]
...
trait OAuth1TokenSecretProvider {
  ...
  def build[B](
      ...
      ec: ExecutionContext): Future[Secret]
  ...
  def publish[B](
      result: Result,
      secret: Secret
    )(implicit request: ExtractableRequest[B]): Result
}
