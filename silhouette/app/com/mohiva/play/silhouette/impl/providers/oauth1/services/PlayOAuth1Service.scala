...
/**
  ...
  */
class PlayOAuth1Service(
    service: OAuth,
    val settings: OAuth1Settings)
    extends OAuth1Service
    with Logger {
  ...
  override def retrieveRequestToken(callbackURL: String)(implicit ec: ExecutionContext): Future[OAuth1Info] =
    Future(service.retrieveRequestToken(settings.callbackURL))
      .map(_.fold(e => throw e, t => OAuth1Info(t.token, t.secret)))
  ...
  /**
    ...
    * @return A OAuth1Info in case of success, Exception otherwise.
    */
  override def retrieveAccessToken(
      oAuthInfo: OAuth1Info,
      verifier: String
    )(implicit ec: ExecutionContext): Future[OAuth1Info] =
    Future(service.retrieveAccessToken(RequestToken(oAuthInfo.token, oAuthInfo.secret), verifier))
      .map(_.fold(e => throw e, t => OAuth1Info(t.token, t.secret)))
...
object PlayOAuth1Service {
  ...
  /**
    ...
    */
  def serviceInfo(settings: OAuth1Settings) =
    ServiceInfo(settings.requestTokenURL,
