          ...
          (json \ "error").asOpt[JsString] match {
            case Some(error) =>
              ...
              throw new ProfileRetrievalException(
                SpecifiedProfileError.format(id, errorDescription, error, response.status)
              )
            case _ =>
...
class Auth0ProfileParser extends SocialProfileParser[JsValue, CommonSocialProfile, OAuth2Info] {
  ...
  override def parse(
      json: JsValue,
      authInfo: OAuth2Info): Future[CommonSocialProfile] =
    Future.successful {
      val userID    = (json \ "user_id").as[String]
      val avatarURL = (json \ "picture").asOpt[String]
      val email     = (json \ "email").asOpt[String]

      CommonSocialProfile(loginInfo = LoginInfo(ID, userID), avatarURL = avatarURL, email = email)
    }
}
