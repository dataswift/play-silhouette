...
/**
  ...
  */
class FoursquareProfileParser(settings: OAuth2Settings)
  ...
  override def parse(
    ...
    Future.successful {
      ...
      CommonSocialProfile(
        ...
        lastName = lastName,
        avatarURL = for {
          prefix <- avatarURLPart1
          postfix <- avatarURLPart2
        } yield prefix + resolution + postfix,
        email = email
      )
    }
}
...
class FoursquareProvider(
    ...
    extends BaseFoursquareProvider
...
object FoursquareProvider {
  ...
  val SpecifiedProfileError =
    "[Silhouette][%s] Error retrieving profile information. Error code: %s, type: %s, detail: %s"
