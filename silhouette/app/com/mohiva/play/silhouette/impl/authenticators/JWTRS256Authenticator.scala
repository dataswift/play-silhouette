...
object JWTRS256Authenticator {
  ...
  def serialize(
    ...
    )(implicit dyn: DynamicSecureEnvironment): String = {
  ...
  def unserialize(
      ...
      authenticatorEncoder: AuthenticatorEncoder,
      settings: JWTRS256AuthenticatorSettings
    )(implicit dyn: DynamicSecureEnvironment): Try[JWTRS256Authenticator] =
    Try {
...
class JWTRS256AuthenticatorService[D <: DynamicSecureEnvironment](
  ...
  /**
    ...
    */
  override def retrieve[B](
      implicit request: ExtractableRequest[B],
      dyn: D): Future[Option[JWTRS256Authenticator]] =
    Future
      .fromTry(Try(request.extractString(settings.fieldName, settings.requestParts)))
      .flatMap {
        case Some(token) =>
          JWTRS256Authenticator.unserialize(token, authenticatorEncoder, settings) match {
  ...
  override def init(
      ...
      dyn: D): Future[String] =
    repository
      .fold(Future.successful(authenticator))(_.add(authenticator))
      .map { a =>
  ...
  override def update(
      ...
      dyn: D): Future[AuthenticatorResult] =
    repository
      .fold(Future.successful(authenticator))(_.update(authenticator))
      .map { a =>
        AuthenticatorResult(
          result.withHeaders(settings.fieldName -> JWTRS256Authenticator.serialize(a, authenticatorEncoder, settings))
        )
      }
      .recover {
  ...
  override def renew(
      ...
      dyn: D): Future[String] =
    repository
      .fold(Future.successful(()))(_.remove(authenticator.id))
      .flatMap { _ =>
