 ...
 */
case class JWTRS256Authenticator(
  id: String,
  loginInfo: LoginInfo,
 ...
 */
object JWTRS256Authenticator {

  val keyUtils = new KeyUtils()
  ...
  def serialize(
    ...
    settings: JWTRS256AuthenticatorSettings)(implicit dyn: DynamicSecureEnvironment): String = {
    ...
    val asymmetricSigningInfo = new SimpleAsymmetricSigningInfo(SigningAlgorithm.RS256, dyn.privateKey)
    new NimbusJwtWriterFactory()
      .signingWriter(asymmetricSigningInfo)
  ...
  def unserialize(
    str: String,
    authenticatorEncoder: AuthenticatorEncoder,
    settings: JWTRS256AuthenticatorSettings)(implicit dyn: DynamicSecureEnvironment): Try[JWTRS256Authenticator] = {

    Try {
      val verifier = new RSASSAVerifier(dyn.publicKey)
 ...
 */
class JWTRS256AuthenticatorService[D <: DynamicSecureEnvironment](
  settings: JWTRS256AuthenticatorSettings,
  repository: Option[AuthenticatorRepository[JWTRS256Authenticator]],
  authenticatorEncoder: AuthenticatorEncoder,
  idGenerator: IDGenerator,
  clock: Clock)(implicit val executionContext: ExecutionContext)
  extends AuthenticatorService[JWTRS256Authenticator, D]
  with Logger {
   ...
   */
  override def create(loginInfo: LoginInfo)(implicit request: RequestHeader): Future[JWTRS256Authenticator] = {
   ...
   */
  override def retrieve[B](implicit request: ExtractableRequest[B], dyn: D): Future[Option[JWTRS256Authenticator]] = {
    Future.fromTry(Try(request.extractString(settings.fieldName, settings.requestParts))).flatMap {
      case Some(token) => JWTRS256Authenticator.unserialize(token, authenticatorEncoder, settings) match {
   ...
   */
  override def init(authenticator: JWTRS256Authenticator)(implicit request: RequestHeader, dyn: D): Future[String] = {
    repository.fold(Future.successful(authenticator))(_.add(authenticator)).map { a =>
      JWTRS256Authenticator.serialize(a, authenticatorEncoder, settings)
    }.recover {
      case e =>
        logger.error(s"Error while initialising authenticator ${ID}, ${authenticator}: ${e.getMessage}", e)
        throw new AuthenticatorInitializationException(InitError.format(ID, authenticator), e)
    }
  }
   ...
   */
  override def touch(authenticator: JWTRS256Authenticator): Either[JWTRS256Authenticator, JWTRS256Authenticator] = {
   ...
   */
  override def update(authenticator: JWTRS256Authenticator, result: Result)(
    implicit
    request: RequestHeader, dyn: D): Future[AuthenticatorResult] = {

    repository.fold(Future.successful(authenticator))(_.update(authenticator)).map { a =>
      AuthenticatorResult(result.withHeaders(settings.fieldName -> JWTRS256Authenticator.serialize(a, authenticatorEncoder, settings)))
    }.recover {
   ...
   */
  override def renew(authenticator: JWTRS256Authenticator)(implicit request: RequestHeader, dyn: D): Future[String] = {
   ...
   * @return The original or a manipulated result.
   */
  override def renew(authenticator: JWTRS256Authenticator, result: Result)(
    implicit
    request: RequestHeader, dyn: D): Future[AuthenticatorResult] = {
   ...
   * @return The manipulated result.
   */
  override def discard(authenticator: JWTRS256Authenticator, result: Result)(
    implicit
 ...
 */
object JWTRS256AuthenticatorService {
 ...
 * @param publicKey The Public Key to sign the JWT.
 */
case class JWTRS256AuthenticatorSettings(
