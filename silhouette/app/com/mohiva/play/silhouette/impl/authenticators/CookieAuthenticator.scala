...
object CookieAuthenticator extends Logger {
  ...
  private def buildAuthenticator(str: String): Try[CookieAuthenticator] =
    Try(Json.parse(str)) match {
      case Success(json) =>
        json.validate[CookieAuthenticator].asEither match {
...
class CookieAuthenticatorService[D <: DynamicEnvironment](
  ...
  override def retrieve[B](
      ...
      dyn: D): Future[Option[CookieAuthenticator]] =
    Future
      .fromTry(Try {
        ...
        request.cookies.get(settings.cookieName) match {
          case Some(cookie) =>
            (repository match {
              ...
              case None =>
                unserialize(cookie.value, signer, authenticatorEncoder) match {
    ...
    (repository match {
      ...
      AuthenticatorResult(
        result.discardingCookies(
          DiscardingCookie(name = settings.cookieName,
                           path = settings.cookiePath,
                           domain = settings.cookieDomain,
                           secure = settings.secureCookie
          )
