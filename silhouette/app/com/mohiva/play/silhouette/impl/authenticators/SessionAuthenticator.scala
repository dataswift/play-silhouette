...
object SessionAuthenticator extends Logger {
  ...
  def unserialize(
      str: String,
      authenticatorEncoder: AuthenticatorEncoder): Try[SessionAuthenticator] =
  ...
  private def buildAuthenticator(str: String): Try[SessionAuthenticator] =
    Try(Json.parse(str)) match {
      case Success(json) =>
        json.validate[SessionAuthenticator].asEither match {
...
class SessionAuthenticatorService(
  ...
  override def retrieve[B](
      ...
      dyn: DynamicEnvironment): Future[Option[SessionAuthenticator]] =
    Future
      .fromTry(Try {
  ...
  override def init(
      ...
      dyn: DynamicEnvironment): Future[Session] =
