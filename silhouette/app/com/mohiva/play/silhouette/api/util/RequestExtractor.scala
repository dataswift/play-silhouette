...
trait RequestExtractor[-B] extends Logger {
  ...
  def extractString(
    ...
    )(implicit request: Request[B]): Option[String]
  ...
  /**
    ...
    * @return The extracted value as string.
    */
  protected def fromQueryString(
      name: String,
      parts: Option[Parts]
    )(implicit request: Request[B]): Option[String] =
    isAllowed(RequestPart.QueryString, parts) {
      logger.debug(
        "[Silhouette] Try to extract value with name `%s` from query string: %s".format(name, request.rawQueryString)
      )
  ...
  /**
    ...
    * @return The extracted value as string.
    */
  protected def fromFormUrlEncoded(
      name: String,
      body: Map[String, Seq[String]],
      parts: Option[Parts]): Option[String] =
  ...
  /**
    ...
    * @return The extracted value.
    */
  protected def fromDefaultParts(
      name: String,
      parts: Option[Parts]
    )(implicit request: Request[B]): Option[String] =
...
trait LowPriorityRequestExtractors {
  ...
  /**
    ...
    */
  implicit def anyExtractor[B]: RequestExtractor[B] =
    new RequestExtractor[B] {
      def extractString(
          name: String,
          parts: Option[Parts] = None
...
class ExtractableRequest[B](request: Request[B])(implicit extractor: RequestExtractor[B])
  ...
  def extractString(
      name: String,
      parts: Option[Seq[RequestPart.Value]] = None): Option[String] =
...
object ExtractableRequest {
  ...
  def apply[B](
      implicit request: Request[B],
      extractor: RequestExtractor[B]) = new ExtractableRequest(request)
  ...
  /**
    ...
    * @return An extractable request.
    */
  implicit def convertImplicit[B](
      implicit request: Request[B],
      extractor: RequestExtractor[B]): ExtractableRequest[B] =
