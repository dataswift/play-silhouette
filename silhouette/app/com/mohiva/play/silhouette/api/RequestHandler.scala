...
trait RequestHandlerBuilder[E <: Env, +R[_]] extends ExecutionContextProvider {
  ...
  /**
    ...
    */
  implicit protected class ExtractEither[T](r: Either[T, T]) {
  ...
  /**
    ...
    */
  final def apply[T](
      block: R[AnyContent] => Future[HandlerResult[T]]
    )(implicit request: Request[AnyContent]): Future[HandlerResult[T]] =
    invokeBlock(block)
  ...
  /**
    ...
    * @return A handler result.
    */
  protected def handleBlock[T](
      authenticator: Either[E#A, E#A],
      block: E#A => Future[HandlerResult[T]]
  ...
  protected def handleAuthentication[B](
      ...
      dyn: E#D): Future[(Option[Either[E#A, E#A]], Option[E#I], E#D)] =
    environment.authenticatorService.retrieve.flatMap {
      ...
      case None =>
        handleRequestProviderAuthentication.flatMap {
          // Authentication was successful, so we retrieve the identity and create a new authenticator for it
          case Some(loginInfo) =>
            environment.identityService.retrieve(loginInfo).flatMap { i =>
  ...
  /**
    ...
    * @return A handler result.
    */
  private def handleInitializedAuthenticator[T](
      authenticator: E#A,
      block: E#A => Future[HandlerResult[T]]
    )(implicit request: RequestHeader,
      dyn: E#D) = {
  ...
  /**
    ...
    * @return A handler result.
    */
  private def handleUninitializedAuthenticator[T](
      authenticator: E#A,
      block: E#A => Future[HandlerResult[T]]
    )(implicit request: RequestHeader,
      dyn: E#D) =
    block(authenticator).flatMap {
      case hr @ HandlerResult(pr: AuthenticatorResult, _) => Future.successful(hr)
      case hr @ HandlerResult(pr, _) =>
        environment.authenticatorService
          .init(authenticator)
          .flatMap { value =>
  ...
  /**
    ...
    */
  private def handleRequestProviderAuthentication[B](
      implicit request: Request[B],
      dyn: E#D): Future[Option[LoginInfo]] = {
    def auth(providers: Seq[RequestProvider[E#D]]): Future[Option[LoginInfo]] =
      providers match {
        case Nil => Future.successful(None)
        case h :: t =>
          h.authenticate(request).flatMap {
