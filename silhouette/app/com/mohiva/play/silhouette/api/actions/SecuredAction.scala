...
case class SecuredRequestHandlerBuilder[E <: Env](
  ...
  /**
    ...
    */
  override def invokeBlock[B, T](
      block: SecuredRequest[E, B] => Future[HandlerResult[T]]
    )(implicit
  ...
  /**
    ...
    */
  private def withAuthorization[B](
      result: Future[(Option[Either[E#A, E#A]], Option[E#I], E#D)]
    )(implicit request: Request[B]) =
    result.flatMap {
      case (Some(a), Some(i), dyn) =>
        authorization
          .map(_.isAuthorized(i, a.extract, dyn))
          .getOrElse(Future.successful(true))
          .map(b => (Some(a), Some(i), Some(b)))
      case (a, i, _) =>
...
/**
  ...
  * @param errorHandler The instance of the secured error handler.
  */
class DefaultSecuredRequestHandler @Inject() (val errorHandler: SecuredErrorHandler) extends SecuredRequestHandler {
...
case class SecuredActionBuilder[E <: Env, P](
  ...
  /**
    ...
    */
  def apply(authorization: Authorization[E#I, E#A, E#D]) =
    SecuredActionBuilder[E, P](requestHandler(authorization), parser)
  ...
  /**
    ...
    * @return A handler result.
    */
  override def invokeBlock[B](
      request: Request[B],
      block: SecuredRequest[E, B] => Future[Result]) = {
