      ...
      failed[ProfileRetrievalException](provider.retrieveProfile(oAuthInfo)) {
        case e =>
          e.getMessage must equalTo(
                SpecifiedProfileError.format(provider.id,
                                             0,
    ...
    "return the social profile" in new WithApplication with Context {
      ...
      profile(provider.retrieveProfile(oAuthInfo)) { p =>
        p must be equalTo CommonSocialProfile(
          ...
          avatarURL = Some(
            "http://media.linkedin.com/mpr/mprx/0_fsPnURNRhLhk_Ue2fjKLUZkB2FL6TOe2S4bdUZz61GA9Ysxu_y_sz4THGW5JGJWhaMleN0F61-Dg"
          )
        )
      }
    }
