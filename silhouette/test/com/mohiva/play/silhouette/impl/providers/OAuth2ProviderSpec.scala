...
abstract class OAuth2ProviderSpec extends SocialStateProviderSpec[OAuth2Info, SocialStateItem] {
  ...
  "The `authenticate` method" should {
    ...
    "fail with an ConfigurationException if authorization URL is undefined when it's needed" in new WithApplication {
      c.oAuthSettings.authorizationURL match {
        ...
        case Some(_) =>
          ...
          c.stateProvider.unserialize(anyString)(any[ExtractableRequest[String]], any[ExecutionContext]) returns Future
            .successful(c.state)
        ...
        case Some(authorizationURL) =>
          ...
          result(c.provider.authenticate()) { result =>
            ...
            redirectLocation(result) must beSome.which { url =>
              ...
              val params = c.oAuthSettings.scope.foldLeft(
                List((ClientID, c.oAuthSettings.clientID),
                     (ResponseType, Code),
                     (State, urlParams(State))
                ) ++ c.oAuthSettings.authorizationParams.toList ++ redirectParam
              ) {
                case (p, s) => (Scope, s) :: p
              }
              url must be equalTo (authorizationURL + params
                .map { p =>
    ...
    "resolves path relative redirectURLs before starting the flow" in new WithApplication {
      verifyRelativeRedirectResolution("redirect-url",
                                       secure = false,
                                       "http://www.example.com/request-path/redirect-url"
      )
    }
    ...
    def verifyRelativeRedirectResolution(
        redirectURL: String,
        secure: Boolean,
    ...
    def verifyPresenceOrAbsenceOfRedirectURL(
    ...
    "submit the proper params to the access token post request" in new WithApplication {
      ...
      val params = Map(ClientID -> Seq(c.oAuthSettings.clientID),
                       ClientSecret -> Seq(c.oAuthSettings.clientSecret),
                       GrantType -> Seq(AuthorizationCode),
                       Code -> Seq("my.code")
        ) ++ c.oAuthSettings.accessTokenParams.mapValues(Seq(_)) ++ redirectParam.toMap.mapValues(Seq(_))
      ...
      failed[UnexpectedResponseException](c.provider.authenticate()) {
        case e =>
          e.getMessage must startWith(
                JsonParseError.format(c.provider.id,
                                      "<html></html>",
                                      "java.lang.RuntimeException: Unexpected character ('<' (code 60))"
                )
              )
      }
    }
