...
class JWTAuthenticatorSpec extends PlaySpecification with Mockito with JsonMatchers with NoLanguageFeatures {
  ...
  "The `serialize` method of the authenticator" should {
    ...
    "throw an AuthenticatorException if a reserved claim will be overridden" in new WithApplication with Context {
      ...
      serialize(authenticator.copy(customClaims = Some(claims)), authenticatorEncoder, settings) must throwA[
    ...
    "throw an AuthenticatorException if an unexpected value was found in the arbitrary claims" in new WithApplication
      with Context {
      val claims = Json.obj(
        "null" -> JsNull
      )
  ...
  "The `unserialize` method of the authenticator" should {
    ...
    "unserialize a JWT" in new WithApplication with Context {
      ...
      unserialize(jwt, authenticatorEncoder, settings) must beSuccessfulTry.withValue(
            authenticator.copy(
  ...
  "The `retrieve` method of the service" should {
    ...
    "return authenticator if DAO is enabled and an authenticator is stored for the token located in the the header" in new WithApplication
      with Context {
      implicit val dyn = FakeDynamicEnvironment()
      implicit val request =
        FakeRequest().withHeaders(settings.fieldName -> serialize(authenticator, authenticatorEncoder, settings))

      repository.find(authenticator.id) returns Future.successful(
            Some(
              authenticator.copy(
                expirationDateTime = authenticator.expirationDateTime.withMillisOfSecond(0),
                lastUsedDateTime = authenticator.lastUsedDateTime.withMillisOfSecond(0)
      ...
      await(service(Some(repository)).retrieve) must beSome(
            authenticator.copy(
              expirationDateTime = authenticator.expirationDateTime.withMillisOfSecond(0),
              lastUsedDateTime = authenticator.lastUsedDateTime.withMillisOfSecond(0)
            )
    ...
    "return authenticator if DAO is enabled and an authenticator is stored for the token located in the the query string" in new WithApplication
      with Context {
      implicit val dyn = FakeDynamicEnvironment()
      implicit val request =
        FakeRequest("GET", s"?${settings.fieldName}=${serialize(authenticator, authenticatorEncoder, settings)}")
    ...
    "return authenticator if DAO is disabled and authenticator was found in the header" in new WithApplication
      with Context {
      implicit val dyn = FakeDynamicEnvironment()
      implicit val request =
        FakeRequest().withHeaders(settings.fieldName -> serialize(authenticator, authenticatorEncoder, settings))

      await(service(None).retrieve) must beSome(
            authenticator.copy(
              expirationDateTime = authenticator.expirationDateTime.withMillisOfSecond(0),
              lastUsedDateTime = authenticator.lastUsedDateTime.withMillisOfSecond(0)
            )
    ...
    "return authenticator if DAO is disabled and authenticator was found in the query string" in new WithApplication
      with Context {
      implicit val request =
        FakeRequest("GET", s"?${settings.fieldName}=${serialize(authenticator, authenticatorEncoder, settings)}")
      implicit val dyn = FakeDynamicEnvironment()
    ...
    "throws an AuthenticatorRetrievalException exception if an error occurred during retrieval" in new WithApplication
      with Context {
      implicit val dyn = FakeDynamicEnvironment()
      implicit val request =
        FakeRequest().withHeaders(settings.fieldName -> serialize(authenticator, authenticatorEncoder, settings))
  ...
  "The `init` method of the service" should {
    "return the token if DAO is enabled and authenticator could be saved in backing store" in new WithApplication
      with Context {
  ...
  "The request `embed` method of the service" should {
    "return the request with a header " in new WithApplication with Context {
      ...
      unserialize(request.headers.get(settings.fieldName).get,
                  authenticatorEncoder,
                  settings
      ).get must be equalTo authenticator
    }
  ...
  "The `update` method of the service" should {
    ...
    "return the result if the authenticator could be stored in backing store" in new WithApplication with Context {
      ...
      unserialize(header(settings.fieldName, result).get,
                  authenticatorEncoder,
                  settings
      ).get must be equalTo authenticator
      there was one(repository).update(authenticator)
    ...
    "throws an AuthenticatorUpdateException exception if an error occurred during update" in new WithApplication
      with Context {
      ...
      await(service(Some(repository)).update(authenticator, Results.Ok)) must throwA[AuthenticatorUpdateException]
            .like {
              case e =>
  ...
  "The `renew` method of the service" should {
    "renew the authenticator and return the response with a new JWT if DAO is enabled" in new WithApplication
      with Context {
      implicit val request = FakeRequest()
      implicit val dyn     = FakeDynamicEnvironment()
      ...
      unserialize(header(settings.fieldName, result).get,
                  authenticatorEncoder,
                  settings
      ).get must be equalTo authenticator.copy(
            id = id,
    ...
    "renew the authenticator and return the response with a new JWT if DAO is disabled" in new WithApplication
      with Context {
      implicit val request = FakeRequest()
      implicit val dyn     = FakeDynamicEnvironment()
      val now              = new DateTime(2015, 2, 25, 19, 0, 0, 0)
    ...
    "throws an AuthenticatorRenewalException exception if an error occurred during renewal" in new Context {
      ...
      await(service(Some(repository)).renew(authenticator, Results.Ok)) must throwA[AuthenticatorRenewalException]
            .like {
              case e =>
  ...
  "The `discard` method of the service" should {
    ...
    "throws an AuthenticatorDiscardingException exception if an error occurred during discarding" in new Context {
      ...
      await(service(Some(repository)).discard(authenticator, okResult)) must throwA[AuthenticatorDiscardingException]
            .like {
              case e =>
