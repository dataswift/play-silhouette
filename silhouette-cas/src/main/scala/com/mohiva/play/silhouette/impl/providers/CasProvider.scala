...
trait BaseCasProvider extends SocialProvider with CasProviderConstants with Logger {
  ...
  def authenticate[B]()(implicit request: ExtractableRequest[B]): Future[Either[Result, CasInfo]] =
    Future.successful {
      request
        .extractString(CasClient.ServiceTicketParameter)
        .map { ticket =>
  ...
  override protected def buildProfile(authInfo: CasInfo): Future[Profile] =
    client
      .validateServiceTicket(authInfo.ticket)
      .flatMap { principal =>
        profileParser.parse(principal, authInfo)
      }
      .transform(identity, error => new ProfileRetrievalException(SpecifiedProfileError.format(id, error.getMessage)))
}
...
/**
  * The CAS protocol.
  */
case class CasProtocol(
    protocol: Protocol,
    ticketValidatorFactory: CasSettings => TicketValidator)
...
object CasProtocol extends Enumeration {
  ...
  /**
    ...
    */
  def apply(protocol: CasProtocol.Value) =
    protocol match {
      case CasProtocol.CAS10 => new CasProtocol(Protocol.CAS1, casValidatorWithEncoding(new Cas10TicketValidator(_)))
      case CasProtocol.CAS20 =>
        new CasProtocol(Protocol.CAS2, casValidatorWithEncoding(new Cas20ServiceTicketValidator(_)))
      case CasProtocol.CAS30 =>
        new CasProtocol(Protocol.CAS3, casValidatorWithEncoding(new Cas30ServiceTicketValidator(_)))
      case CasProtocol.SAML =>
        new CasProtocol(Protocol.SAML11,
                        { settings =>
